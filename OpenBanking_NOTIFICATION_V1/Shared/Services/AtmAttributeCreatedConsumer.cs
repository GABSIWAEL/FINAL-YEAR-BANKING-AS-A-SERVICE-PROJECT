...
{
    public class AtmAttributeCreatedConsumer : BackgroundService
    {
        private readonly IConfiguration _config;
        private readonly EmailSenderService _emailSender;

        public AtmAttributeCreatedConsumer(IConfiguration config, EmailSenderService emailSender)
        {
            _config = config;
        ...
        {
            await ListenAsync(
                "notification_atmattribute_queue",
                "atm.attribute.created",
                async (msg) =>
                {
                    var evt = JsonConvert.DeserializeObject<AtmAttributeCreatedEvent>(msg);
                    await _emailSender.SendAtmAttributeCreatedEmail(evt);
                },
                stoppingToken
            );
        }
